...
This Java program provides four conversion tools using a command-line interface:
...
1. The first two integers are the width and height.
2. Every pixel is represented by three integers in red, green, blue order.
3. Each RGB component must be between `0` and `255`.
4. The file must provide exactly enough RGB groups to fill the specified image dimensions.
